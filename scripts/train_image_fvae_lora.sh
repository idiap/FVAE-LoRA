...
fine_tuning_method="fvae_peft" # fvae_peft
...
fvae_lambda_downstream=1000
fvae_lambda_reconstruction=1 #0.1, 1
fvae_lambda_kl_z1=1 # 1, 10
fvae_lambda_z2_l2=1
fvae_lambda_z1_l2=1

paramtag="d${fvae_lambda_downstream}_r${fvae_lambda_reconstruction}_kl${fvae_lambda_kl_z1}_z2l2${fvae_lambda_z2_l2}_z1l2${fvae_lambda_z1_l2}"
...
for i in "${!dataset_list[@]}" ; do
    ...
    exp_dir="exp/exp_image/${fine_tuning_method}/${model_path}/${dataset_alias_name}/${seed}/${paramtag}"
    mkdir -p ${exp_dir}
    tensorboard_logdir="${exp_dir}/tensorboard_logs/"

    cmd="sbatch --account <your-project> --job-name "$fine_tuning_method"_"$dataset_alias_name"_"$seed"_"$model_path" --partition=gpu --gpus=rtx3090:1 --ntasks=1"
    ...
    $cmd --wrap="python image_main.py \
        ...
        --fvae_lambda_downstream ${fvae_lambda_downstream} \
        --fvae_lambda_reconstruction ${fvae_lambda_reconstruction} \
        --fvae_lambda_kl_z1 ${fvae_lambda_kl_z1} \
        --fvae_lambda_z2_l2 ${fvae_lambda_z2_l2} \
        --fvae_lambda_z1_l2 ${fvae_lambda_z1_l2} \
        --tensorboard_use \
